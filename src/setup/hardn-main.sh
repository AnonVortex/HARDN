#!/bin/bash
# First Established by: Chris B. > the Rice Master. 
...
grub_security() {
    ...
    if systemd-detect-virt --quiet --vm; then
        ...
        echo "[INFO] GRUB bootloader security is typically managed by the VM host system."
        return 0
    fi

    # Check system type
    if [ -d /sys/firmware/efi ]; then
        echo "[INFO] Detected UEFI boot system"
        echo "[INFO] GRUB security configuration is not required for UEFI systems."
        return 0
    else
...
# HARDN-XDR Kernel Security Configuration
...
pen_test() {
    HARDN_STATUS "info" "Running security audit with Lynis and nmap..."
    ...
    HARDN_STATUS "info" "Running Lynis system audit..."
    ...
    HARDN_STATUS "info" "Generating remediation report..."
...
if [[ $# -gt 0 ]]; then
    case "$1" in
        ...
        --help|-h)
            ...
            echo ""
            echo "This script applies security hardening to Debian-based systems"
        ...
        --non-interactive)
            ...
            HARDN_STATUS "info" "Running in non-interactive mode for automation"
            ;;
        *)
